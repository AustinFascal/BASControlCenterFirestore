...
public class ManageReceivedOrderActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        roManagementAdapter = new ROManagementAdapter(this, receivedOrderModelArrayList);


        btnDeleteSelected.setOnClickListener(view -> {
            int size = roManagementAdapter.getSelected().size();
        ...
        Runnable runnable = new Runnable() {
            public void run() {
                int itemSelectedSize = roManagementAdapter.getSelected().size();
                if (roManagementAdapter.getSelected().size()>0){
        ...
        edtRoFilterDateStart.setOnClickListener(view -> {
            ...
            datePicker = new DatePickerDialog(ManageReceivedOrderActivity.this,
            ...
            if (!value.isEmpty()){
                ...
                for (DocumentSnapshot d : value.getDocuments()) {
                    ...
                    if (searchTypeData.equals(searchTypeValue[1])){
                        if(Objects.requireNonNull(receivedOrderModel).getRoUID().contains(newText)) {
                            receivedOrderModelArrayList.add(receivedOrderModel);
                        }
                    }

                    if (searchTypeData.equals(searchTypeValue[2])){
                        if(Objects.requireNonNull(receivedOrderModel).getRoPoCustNumber().contains(newText)) {
                            receivedOrderModelArrayList.add(receivedOrderModel);
                        }
                    }
                }
        ...
        query.addSnapshotListener(new EventListener<QuerySnapshot>() {
            ...
            public void onEvent(@Nullable QuerySnapshot value, @Nullable FirebaseFirestoreException error) {
                ...
                if (!value.isEmpty()){
                    for (DocumentSnapshot d : value.getDocuments()) {
                        ReceivedOrderModel receivedOrderModel = d.toObject(ReceivedOrderModel.class);
                        if(Objects.requireNonNull(receivedOrderModel).getRoType()==data) {
                            receivedOrderModelArrayList.add(receivedOrderModel);
                            llNoData.setVisibility(View.GONE);
                            nestedScrollView.setVisibility(View.VISIBLE);
                        }
                ...
                if (receivedOrderModelArrayList.size()<1){
                    nestedScrollView.setVisibility(View.GONE);
                    llNoData.setVisibility(View.VISIBLE);
                }
            }
